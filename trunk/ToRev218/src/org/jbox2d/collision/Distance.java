...
public class Distance{
	...
	private static final Vec2 p1s[] = new Vec2[3];
	private static final Vec2 p2s[] = new Vec2[3];
	private static final Vec2 points[] = new Vec2[3];
	private static final Vec2 v = new Vec2();
	private static final Vec2 vNeg = new Vec2();
	private static final Vec2 w = new Vec2();
	private static final Vec2 w1 = new Vec2();
	private static final Vec2 w2 = new Vec2();
	...
	protected final static int ProcessTwo(final Vec2 x1, final Vec2 x2, final Vec2[] p1s, final Vec2[] p2s, final Vec2[] points) {
		// If in point[1] region
		Distance.p2r.x = -points[1].x;
		Distance.p2r.y = -points[1].y;
		Distance.p2d.x = points[0].x - points[1].x;
		Distance.p2d.y = points[0].y - points[1].y;
		final float length = Distance.p2d.normalize();
		float lambda = Vec2.dot(Distance.p2r, Distance.p2d);
	...
	// - inside the triangle
	protected final static int ProcessThree(final Vec2 x1, final Vec2 x2, final Vec2[] p1s, final Vec2[] p2s, final Vec2[] points) {
	...
	public final static boolean InPoints(final Vec2 w, final Vec2[] points, final int pointCount) {
	 ...
	 */
	// pooled from above
	public static final float DistanceGeneric(final Vec2 x1, final Vec2 x2,
		...
		for (int iter = 0; iter < maxIterations; ++iter) {
			...
			if (vSqr - vw <= 0.01f * vSqr || Distance.InPoints(w, points, pointCount)) // or w in points
			{
				if (pointCount == 0) {
					x1.set(w1);
					x2.set(w2);
	...
	// djm pooled
	private final static Vec2 distCCp1 = new Vec2();
	private final static Vec2 distCCp2 = new Vec2();
	private final static Vec2 distCCd = new Vec2();
	 ...
	 * @return the distance
	 */
	public final static float DistanceCC(
	                                  final Vec2 x1, final Vec2 x2,
		...
		final float dSqr = Vec2.dot(Distance.distCCd, Distance.distCCd);
		final float r1 = circle1.getRadius() - Settings.toiSlop;
		final float r2 = circle2.getRadius() - Settings.toiSlop;
		final float r = r1 + r2;
		if (dSqr > r * r){
			final float dLen = Distance.distCCd.normalize();
			final float distance = dLen - r;
			x1.set( Distance.distCCp1.x + r1 * Distance.distCCd.x,
			        Distance.distCCp1.y + r1 * Distance.distCCd.y);
			x2.set( Distance.distCCp2.x - r2 * Distance.distCCd.x,
			        Distance.distCCp2.y - r2 * Distance.distCCd.y);
			return distance;
		} else if (dSqr > Settings.EPSILON * Settings.EPSILON) {
			Distance.distCCd.normalize();
			x1.set( Distance.distCCp1.x + r1 * Distance.distCCd.x,
			        Distance.distCCp1.y + r1 * Distance.distCCd.y);
			x2.set(x1);
			return 0.0f;
		...
		x1.set(Distance.distCCp1);
		x2.set(x1);
		return 0.0f;
	}

	// djm pooled
	private final static Vec2 cWorld = new Vec2(); // just like sea world but with less water and more chlorine
	//private static Vec2 shamoo = new Vec2();
	private final static Vec2 ECcLocal = new Vec2();
	private final static Vec2 ECvWorld = new Vec2();
	private final static Vec2 ECd = new Vec2();
	private final static Vec2 ECtemp = new Vec2();
	 ...
	 * @return the distance
	 */
	public final static float DistanceEdgeCircle(
	                                          final Vec2 x1, final Vec2 x2,
		...
		if (dirDist <= 0.0f) {
			XForm.mulToOut(xf1, edge.getCoreVertex1(), Distance.ECvWorld);
		} else if (dirDist >= edge.getLength()) {
			XForm.mulToOut(xf1, edge.getCoreVertex2(), Distance.ECvWorld);
		} else {
			...
			Distance.ECtemp.set( Distance.ECcLocal);
			Distance.ECtemp.subLocal(edge.getCoreVertex1());
			dLen = Vec2.dot(Distance.ECtemp, edge.getNormalVector());

			if (dLen < 0.0f) {
				if (dLen < -r) {
					x2.set(edge.getNormalVector());
					x2.mulLocal( r).addLocal( Distance.ECcLocal);
				...
				if (dLen > r) {
					x2.set(edge.getNormalVector());
					x2.mulLocal( r).subLocal( Distance.ECcLocal).negateLocal();
					XForm.mulToOut( xf1, x2, x2);
		...
		x1.set(Distance.ECvWorld);
		Distance.ECd.set(Distance.cWorld);
		Distance.ECd.subLocal(Distance.ECvWorld);
		dSqr = Vec2.dot(Distance.ECd, Distance.ECd);
		if (dSqr > r * r) {
			dLen = Distance.ECd.normalize();
			x2.set( Distance.ECd);
			x2.mulLocal( r).subLocal( Distance.cWorld).negateLocal();
			//x2.set(ECcWorld.sub(ECd.mul(r)));
			return dLen - r;
		} else {
			x2.set(Distance.ECvWorld);
			return 0.0f;
		}
	}
	 ...
	 * @return the distance
	 */
	public static final float DistancePC(
	                           final Vec2 x1, final Vec2 x2,
	...
	public static final float DistancePolygonPoint(
	                                     final Vec2 x1, final Vec2 x2,
	                                     final PolygonShape polygon, final XForm xf1,
	                                     final PointShape pt,   final XForm xf2) {
		// v is just used as a dummy Vec2 since it gets overwritten in a moment
	...
	// djm pooled
	private final static Vec2 CPp1 = new Vec2();
	private final static Vec2 CPp2 = new Vec2();
	private final static Vec2 CPd = new Vec2();
	/**
	 ...
	 * @return the distance
	 */
	public final static float DistanceCirclePoint(
	                                           final Vec2 x1, final Vec2 x2,
	                                           final CircleShape circle1, final XForm xf1,
		...
		final float dSqr = Vec2.dot(Distance.CPd, Distance.CPd);
		final float r1 = circle1.getRadius() - Settings.toiSlop;
		final float r2 = -Settings.toiSlop; //this is necessary, otherwise the toi steps aren't taken correctly...
		final float r = r1 + r2;
		if (dSqr > r * r){
			final float dLen = Distance.CPd.normalize();
			final float distance = dLen - r;
			x1.set( Distance.CPp1.x + r1 * Distance.CPd.x,
			        Distance.CPp1.y + r1 * Distance.CPd.y);
			x2.set( Distance.CPp2.x - r2 * Distance.CPd.x,
			        Distance.CPp2.y - r2 * Distance.CPd.y);
			return distance;
		} else if (dSqr > Settings.EPSILON * Settings.EPSILON) {
			Distance.CPd.normalize();
			x1.set( Distance.CPp1.x + r1 * Distance.CPd.x,
			        Distance.CPp1.y + r1 * Distance.CPd.y);
			x2.set(x1);
			return 0.0f;
	 ...
	 * @return the distance
	 */
	public static final float distance(final Vec2 x1, final Vec2 x2,
	                      final Shape shape1, final XForm xf1,
		...
		if (type1 == ShapeType.CIRCLE_SHAPE && type2 == ShapeType.CIRCLE_SHAPE) {
			...
			return Distance.DistanceEdgeCircle(x1, x2, (EdgeShape)shape1, xf1, (CircleShape)shape2, xf2);
		} else if (type1 == ShapeType.CIRCLE_SHAPE && type2 == ShapeType.EDGE_SHAPE) {
			return Distance.DistanceEdgeCircle(x2, x1, (EdgeShape)shape2, xf2, (CircleShape)shape1, xf1);
			...
			return Distance.DistanceCirclePoint(x2, x1, (CircleShape)shape2, xf2, (PointShape)shape1, xf1);
		} else if (type1 == ShapeType.CIRCLE_SHAPE && type2 == ShapeType.POINT_SHAPE) {
			return Distance.DistanceCirclePoint(x1, x2, (CircleShape)shape1, xf1, (PointShape)shape2, xf2);
