...
public class LineJoint extends Joint {
	...
	public LineJoint(WorldPool argPool, LineJointDef def) {
		...
		Vec2.crossToOut(1.0f, def.localAnchorA, m_localXAxis1);
	...
	public void initVelocityConstraints(TimeStep step) {
		...
		// Compute motor Jacobian and effective mass.
		{
			...
			m_motorMass = m_invMassA + m_invMassB + m_invIA * m_a1 * m_a1
					+ m_invIB * m_a2 * m_a2;
		...
		if (step.warmStarting) {
			...
			float L1 = m_impulse.x * m_s1 + (m_motorImpulse + m_impulse.y)
					* m_a1;
			float L2 = m_impulse.x * m_s2 + (m_motorImpulse + m_impulse.y)
					* m_a2;
	...
	public void solveVelocityConstraints(TimeStep step) {
		...
		if (m_enableLimit && m_limitState != LimitState.INACTIVE) {
			...
			// Limit is inactive, just solve the prismatic finalraint in block
			// form.
			float df;
			if (m_K.col1.x != 0.0f) {
				df = -Cdot1 / m_K.col1.x;
